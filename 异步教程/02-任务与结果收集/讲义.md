...
第一种创建完就等待，中间没有别的 `await`，没有利用到独立 Task 的并发能力；直接 `await` 更简单。普通 `print()`、计算、赋值也不会让事件循环切换，只有遇到 `await`、`return` 或异常才有机会推进新 Task。

`create_task()` 只在“先启动，后面还有事要做”时值得用：

```text
创建 Task → await 其他独立工作 → await Task 取结果
```

或者你需要保存句柄以便取消、检查 `done()`、命名或统一回收。若只是同时等多个独立任务，通常直接用 `gather()` 更清楚。
