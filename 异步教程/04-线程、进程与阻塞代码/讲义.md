...
| 写法 | await 何时成功 | 同步函数失败时 | 等待方被取消时 | 是否定义一批工作的整体成败 |
| --- | --- | --- | --- | --- |
| `await to_thread(func)` | 工作线程中的 `func` 正常返回 | 原异常回到事件循环，在 `await` 处抛出 | 等待协程停止等待；**已经运行的线程函数通常不会因此停下** | 否 |
| `await run_in_executor(...)` | Executor 中的调用正常返回 | 原异常在 `await` 处抛出 | Future 会收到取消请求；已开始的函数通常无法强制中断 | 否 |
| `await gather(...)` 包住多个线程/进程 Future | 依照 `gather` 的规则 | 第一个异常传播 | `gather` 被取消时会取消它管理的 awaitable | 是，由 `gather` 定义 |
| `TaskGroup` 包住多个 `to_thread` 调用 | 依照 `TaskGroup` 的规则 | 组失败并取消未完成的等待 Task | 线程内已开始的同步函数仍可能继续 | 是，由 `TaskGroup` 定义 |

`to_thread` 和 Executor 只决定“函数在哪里执行”，不决定“一批函数怎样算成功”。批量成败取决于外层使用的是 `gather`、`TaskGroup`，还是分别 `await`。

## `to_thread()` 与 `create_task()`：不是替代关系

| API | 接收什么 | 实际执行位置 | 它解决的问题 |
| --- | --- | --- | --- |
| `create_task(coro)` | 协程对象，如 `async_work()` | 事件循环（通常同一线程） | 提前启动协程、保留 Task 句柄 |
| `to_thread(func)` | 同步函数，如 `sync_work` | 工作线程 | 不让阻塞同步函数卡住事件循环 |

`to_thread()` 不是把同步函数内部“改写成异步”；它只是把函数放到线程里运行，再把结果做成可 `await` 的形式。同步函数快就先完成，慢就后完成，并不存在“`to_thread` 总是最后执行”。

两者可以组合：

```python
task = asyncio.create_task(asyncio.to_thread(sync_work))
```

这句话的意思是：同步函数在线程执行；外层等待过程由一个 Task 管理。若你写完立刻等待，通常不必创建 Task：

```python
result = await asyncio.to_thread(sync_work)
```

不要写 `asyncio.create_task(sync_work())`：`sync_work()` 会先在事件循环线程同步执行并阻塞，且返回值通常不是协程。也不要把 `async def` 函数交给 `to_thread`；协程函数直接 `await` 或 `create_task` 即可。
...
还有一个非常重要的取消边界：Python 不能安全地从外部强行杀死一个已经运行的线程。若等待 `to_thread` 的 Task 被取消，asyncio 可以让这个 Task 不再等待结果，但工作线程里的 `blocking_io()` 通常仍会执行到函数自己返回。对于必须可停止的长任务，要让同步函数自己周期性检查停止信号；对于有副作用的函数，不能把“协程已取消”误认为“副作用没有发生”。

```mermaid
sequenceDiagram
    participant P as 等待方 Task
    participant W as 工作线程
    participant X as 取消发起方

    P->>W: await to_thread(blocking_io)
    W->>W: 已开始执行同步函数
    X->>P: cancel()
    P-->>X: P 收到 CancelledError 并结束
    Note over W: 线程不能被 asyncio 强制杀死
    W->>W: blocking_io 仍执行到返回
```
