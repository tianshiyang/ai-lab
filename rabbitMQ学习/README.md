# RabbitMQ 入门：在 PyCharm 里运行示例

用 Python 连接服务器上的 RabbitMQ，练习收发消息、失败处理和多队列分发。**所有练习脚本都可以在 PyCharm 里点运行，Parameters 留空。**
...
上次在 2026-09-06 检查时，管理页面正常，但本机访问 5672 超时。先运行连接检查；如果仍超时，按第 2 节排查。
...
已在项目 `.run` 目录准备六个运行配置，选择后点绿色三角即可：
...
每个可调整的脚本顶部都有一小段设置，例如 topology.py：
...
想做积分练习，就改成 `True`，保存，再点运行。这个值是**代码里的布尔值**，不是启动参数。
...
| 写法 | 原来的作用 |
| --- | --- |
| `argparse` | Python 自带的命令行参数工具，与 RabbitMQ 无关 |
| `ArgumentParser(...)` | 创建一个读取启动参数的对象 |
| `description` | 帮助页面的简介，不会创建交换机 |
| `add_argument(...)` | 注册可接受的启动选项 |
| `action="store_true"` | 有这个开关时得到 True，没有时得到 False |
| `help` | 这个选项的帮助文字 |
| `parse_args()` | 实际读取启动时传入的参数 |

现在直接写 `WITH_POINTS = True` 就能表达同一件事，不需要先学这套命令行工具。

**修改设置只影响下次启动的进程。** 已运行的消费者仍使用启动时读到的值。改配置后重新运行；如果要替换旧消费者，先在它的 Run 窗口点红色停止按钮。
...
这是常见的职责划分：入口读取设置，消息模块负责 RabbitMQ，业务模块负责订单。业务函数不持有通道、不调用 ack，这样换消息客户端时更容易保留业务逻辑。

文件顶部的大写设置是练习入口选项，便于你点运行。账号等部署配置仍从 .env 读取。这里没有为了分层再加接口类、继承或依赖注入框架。
...
图是后续接入数据库时的思路，不是当前示例已经实现的功能。重发与重复消费仍可能发生，去重需要业务保证。
